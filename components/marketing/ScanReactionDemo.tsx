...
export default function ScanReactionDemo() {
  ...
  return <section id="scannen" className="section container scan-section" aria-labelledby="scan-title"><div className="section-copy"><h2 id="scan-title">{t("Pokémon-Karten scannen.")}<br /><span className="muted-heading">{t("Besonders, wenn")}<br className="desktop-break" />{" "}{t("es besonders ist.")}</span></h2><p>{t("Füge Karten zu deiner Sammlung hinzu und sieh direkt, was sich verändert. Neue Dex-Einträge, Fortschritt für dein Set und kleine Reaktionen, die zur Karte passen.")}</p><a className="text-button feature-link" href={href("/pokemon-tcg-scanner")}>{t("So funktioniert der Kartenscanner")}</a><div className="scan-explanation"><ScanLine size={20} /><span>{t("Ein kleiner Moment.")}<br />{t("Für deine nächste Lieblingskarte.")}</span></div></div>
