...
Production: https://kujira-py.github.io/mondex-website/
English: https://kujira-py.github.io/mondex-website/en/
Sitemap: https://kujira-py.github.io/mondex-website/sitemap.xml
...
- Each page has a unique title, description, canonical URL and reciprocal `de`, `en`, `x-default` alternate links. English has `/en/` URLs. Existing `?lang=en` browser links move to the corresponding English URL.
...
- On a GitHub **project** site, `/mondex-website/robots.txt` is not the host-root robots file. Crawlers use `https://kujira-py.github.io/robots.txt`. The project does not block indexing; submit its exact sitemap directly. Do not edit the account-root website without separate authorization.

## Publish

`npm ci`, then `npm run build:pages` and `npm run verify:pages`.

The Pages build uses the installed Next.js static exporter and writes `out/`. The preview/Sites workflow can still use Vinext. Deployment defaults are set in `scripts/build-pages.mjs`; `NEXT_PUBLIC_SITE_URL` and `NEXT_PUBLIC_BASE_PATH` can override the production address and project prefix. Keep them consistent when moving to a custom domain.
...
1. Add the URL-prefix property `https://kujira-py.github.io/mondex-website/` in Google Search Console.
