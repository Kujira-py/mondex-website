...
The site is published on GitHub Pages at the custom domain `https://mondextcg.com/`. The launch waitlist form posts directly from the browser to the MonDex API (`/api/v1/waitlist`); its base URL defaults to `https://mondex-api.onrender.com` in `components/marketing/config.ts` and can be overridden at build time with `NEXT_PUBLIC_MONDEX_API_URL`. The API's CORS allowlist only admits `mondextcg.com` and `www.mondextcg.com`, so the form cannot submit from `localhost` unless that origin is added. English is the default; German is available at `/de/`. Run `npm run build:pages` and `npm run verify:pages` for the static site. The public repository workflow publishes `out/` on pushes to main. See `SEO.md` for the route map, indexing policy and Search Console follow-up.
